...
#define CONFIG_MX6Q
#ifdef QSEVEN
#define CONFIG_MX6Q_SECO_Q7
#endif
#ifdef UQSEVEN
#define CONFIG_MX6Q_SECO_UQ7
#endif
#ifdef uSBC
#define CONFIG_MX6Q_SECO_uSBC
#endif
...
#if (CONFIG_DDR_SIZE == 0)
#define MEMORY_SIZE_PARAM	"mem=512M"
#elif (CONFIG_DDR_SIZE == 1) || (CONFIG_DDR_SIZE == 2)
#define MEMORY_SIZE_PARAM	"mem=1024M"
#elif (CONFIG_DDR_SIZE == 3)
#define MEMORY_SIZE_PARAM	"mem=2048M"
#elif (CONFIG_DDR_SIZE == 4)
#define MEMORY_SIZE_PARAM	"mem=3800M"
#endif

#if defined(QSEVEN)
#define uSD_NAME	"mmc3"
#define EXT_SD_NAME	"mmc0"
#define eMMC_NAME	"mmc2"
#elif defined(UQSEVEN)
#define uSD_NAME	"NONE"
#define EXT_SD_NAME	"mmc3"
#define	eMMC_NAME	"mmc2"
#elif defined(uSBC)
#define uSD_NAME	"NONE"
#define EXT_SD_NAME	"NONE"
#define	eMMC_NAME	"mmc2"
#endif

#define USBOTG_NAME "usbotg"
...
#define	CONFIG_EXTRA_ENV_SETTINGS					\
		...
		"fastboot_dev=mmc0\0"					\
		"set_fsdevice="												\
			"if test $bootdev = \"" uSD_NAME "\" \n"							\
				"then\n"										\
					"setenv bootargs ${bootargs} androidboot.fstab=fstab.usd.freescale;\n"		\
			"elif test $bootdev = \"" EXT_SD_NAME "\" \n"							\
				"then\n"										\
					"setenv bootargs ${bootargs} androidboot.fstab=fstab.extsd.freescale;\n"	\
			"elif test $bootdev = \"" eMMC_NAME "\" \n"							\
				"then\n"										\
					"setenv bootargs ${bootargs} androidboot.fstab=fstab.emmc.freescale;\n"		\
			"fi\0"												\
		"splashimage=0x30000000\0"				\
		"splashpos=m,m\0"					\
		"lvds_num=1\0"                   			\
		"memory=" MEMORY_SIZE_PARAM "\0"													\
		"setvideomode=setenv videomode video=mxcfb0:dev=hdmi,1920x1080M@60,if=RGB24,bpp=32 video=mxcfb1:off video=mxcfb2:dev=ldb,${lvdsres1}\0"	\
		"lvdsres1=LDB-WVGA,if=RGB666,bpp=32\0"														\
		"lvdsres2=\0"																\
		"setbootargs=setenv bootargs console=ttymxc1,115200 init=/init ${videomode} fbmem=28M vmalloc=400M androidboot.console=ttymxc1 androidboot.hardware=freescale ${memory}\0"	\
		"bootcmd_std=run setvideomode; run setbootargs; run set_fsdevice; booti $bootdev\0"		\
		"option=rootwait rw fixrtc rootflags=barrier=1\0"		\
		"setbootargs_otg=setenv bootargs console=ttymxc1,115200 ${memory} ${option}\0"	\
		"bootcmd_otg=run setbootargs_otg; bootm 0x27900000 0x27E00000\0"		\
		"bootcmd="		\
			"if test $bootdev = \"" USBOTG_NAME "\" \n"							\
				"then\n"										\
					"run bootcmd_otg;\n"						\
				"else\n"										\
					"run bootcmd_std;\n"						\
...
#ifdef UQSEVEN
	#define CONFIG_SYS_PROMPT		"MX6Q SECO_uQ7 U-Boot > "
#elif defined(QSEVEN)
	#define CONFIG_SYS_PROMPT		"MX6Q SECO_Q7 U-Boot > "
#elif defined(uSBC)
	#define CONFIG_SYS_PROMPT		"MX6Q SECO_uSBC U-Boot > "
#else
	#define CONFIG_SYS_PROMPT		"MX6Q SECO U-Boot > "
#endif
...
#define CONFIG_FASTBOOT_PRODUCT_NAME_STR "i.mx6q SECO SmartDevice"
